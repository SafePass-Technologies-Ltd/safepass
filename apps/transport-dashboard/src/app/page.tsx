/// Transport Dashboard — Login Page
'use client';
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      const data = await apiClient<{ accessToken: string; refreshToken: string }>(
        ...
        { method: 'POST', body: JSON.stringify({ firebaseIdToken: idToken }) }
      );
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md space-y-8 rounded-2xl bg-white p-8 shadow-xl">
        ...
        <button onClick={handleGoogleSignIn} disabled={loading} className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-50">
