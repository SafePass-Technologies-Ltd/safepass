...
/// and activates a plan immediately via wallet deduction (no admin approval step).
///
/// Flow:
///   1. Page loads → GET /v1/org/subscription (plan, slots, wallet balance, available plans)
///   2. Admin selects a plan card (or Custom and enters a slot count)
///   3. For Custom: slot count input debounces → GET /v1/org/subscription/price for live preview
///   4. "Activate Plan" opens a confirmation modal showing amount to be deducted
///   5. Modal confirm → POST /v1/org/subscription/activate
///   6. On success: show confirmation banner with new plan + deducted amount
///   7. On insufficient balance: show top-up prompt with shortfall amount
'use client';
...
function PlanCard({ tier, isCurrent, onSelect }: PlanCardProps) {
  const isHighlighted = tier.plan === 'business';
...
// Custom plan card with slot count input and live price preview
...
function CustomPlanCard({ isCurrent, currentCustomSlots, orgId: _orgId, onSelect }: CustomPlanCardProps) {
  const [slots, setSlots] = useState<string>(String(currentCustomSlots ?? 25));
  ...
  // Debounce slot count input — fetch preview 400ms after user stops typing
  useEffect(() => {
  ...
  // Fetch on mount with default value
  useEffect(() => {
    const num = parseInt(slots, 10);
    if (!isNaN(num)) fetchPreview(num);
    ...
    <div className="relative flex flex-col rounded-2xl border border-slate-200 bg-white p-6 transition-shadow hover:shadow-sm">
      <div className="mb-4">
        ...
        <p className="mt-0.5 text-sm text-slate-500">
          Choose any slot count (min 5). Price calculated by tiered per-slot rates.
        </p>
      </div>

      {/* Slot count input */}
      <div className="mb-4">
        <label className="block text-xs font-medium text-slate-500 mb-1.5" htmlFor="custom-slots">
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className={`mb-5 rounded-xl border p-4 ${sufficient ? 'border-blue-100 bg-blue-50' : 'border-red-100 bg-red-50'}`}>
          <div className="flex items-center justify-between text-sm mb-1.5">
            ...
            <span className={`font-semibold ${sufficient ? 'text-blue-800' : 'text-red-800'}`}>
          ...
          <div className="mt-2 border-t border-current/10 pt-2 flex items-center justify-between text-sm">
            <span className={`font-medium ${sufficient ? 'text-blue-700' : 'text-red-700'}`}>
              Balance after
            </span>
            <span className={`font-bold ${sufficient ? 'text-blue-800' : 'text-red-800'}`}>
              {formatNgn(Math.max(0, walletBalance - priceNgn))}
            </span>
          </div>
        ...
        {sufficient ? (
          <p className="mb-5 text-sm text-slate-500">
            The subscription fee will be deducted from your org wallet immediately and the plan
            will be activated. Renewal is billed monthly from your org wallet.
          </p>
        ) : (
          <Link
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
export default function SubscriptionPage() {
  ...
  // Selected plan and price before the modal is opened
  ...
  function handleNamedPlanSelect(tier: PlanTier) {
    setPendingSelection({
      plan: tier.plan,
      planLabel: tier.label,
      slots: tier.slots,
      priceNgn: tier.monthlyNgn,
    });
    setSuccessResult(null);
    setError(null);
  }

  function handleCustomPlanSelect(slots: number, price: PlanPriceResult) {
    setPendingSelection({
      plan: 'custom',
      planLabel: 'Custom',
      slots,
    ...
    try {
      const result = await apiClient<ActivateResult>('/v1/org/subscription/activate', {
        ...
        body: JSON.stringify({
          ...
          ...(pendingSelection.plan === 'custom'
            ? { customSlotCount: pendingSelection.slots }
            : {}),
        }),
      });
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Manage your organisation&apos;s monitoring plan and member slots.
        </p>
      </div>
      ...
      {/* Current plan + wallet balance row */}
      <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            {data.subscriptionPlan !== 'none' && (
              <p className="mt-0.5 text-sm text-slate-500">
                ...
                {data.subscriptionPlan === 'custom' && data.customSlotCount
                  ? ` (custom — ${data.customSlotCount} slots)`
                  : ''}
              </p>
            )}
          </div>
        ...
        <div className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-lg font-bold text-slate-dark">
              {data.walletBalanceFormatted}
            </p>
      ...
      {/* Legacy pending request notice (admin-approval path) */}
      {data.status === 'pending' && data.pendingRequest && (
        <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
          <Clock className="mt-0.5 h-5 w-5 flex-shrink-0 text-amber-600" />
          <div>
            <p className="text-sm font-semibold text-amber-800">Plan request under review</p>
            <p className="mt-0.5 text-sm text-amber-700">
              Your request for the{' '}
              <span className="font-semibold capitalize">{data.pendingRequest.requestedPlan}</span>{' '}
              plan ({data.pendingRequest.requestedSlotCount} slots) is awaiting admin review.
            </p>
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <p className="mb-4 text-sm text-slate-500">
          Plans are billed monthly from your org wallet. Activation is immediate.
        </p>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {data.availablePlans.map((tier) => (
            <PlanCard
              ...
              onSelect={() => handleNamedPlanSelect(tier)}
            />
          ))}

          {/* Custom plan card always shown as the last option */}
          <CustomPlanCard
            isCurrent={data.subscriptionPlan === 'custom'}
            currentCustomSlots={data.customSlotCount}
            orgId={data.orgId}
            onSelect={handleCustomPlanSelect}
          />
        </div>
      </div>

      {/* Pricing algorithm note */}
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
        <p className="font-medium text-slate-700 mb-1">Custom plan pricing</p>
        <p>
          Per-slot monthly rates: 5–19 slots ₦4,000/slot &middot; 20–49 slots ₦3,500/slot &middot;
          50–99 slots ₦3,000/slot &middot; 100+ slots ₦2,500/slot. Named plans are always priced
          below the Custom equivalent — choose a named plan to save.
        </p>
      </div>
