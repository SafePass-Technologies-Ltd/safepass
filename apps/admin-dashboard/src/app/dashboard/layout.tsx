...
/// - Main content area
/// - Global sticky message notification toasts (bottom-right)
...
export default function DashboardLayout({
  ...
  // Forward new traveller messages to the toast component via a window-level
  // handler that MessageNotificationToast registers on mount.
  // Also dispatch a CustomEvent so individual pages can independently react
  // (e.g. increment unread counts) without clobbering the toast's handler slot.
  const handleNewMessage = useCallback((msg: IncomingMessage) => {
    const handler = (
      window as Window & { __spMessageHandler?: (m: IncomingMessage) => void }
    ).__spMessageHandler;
    handler?.(msg);

    // Broadcast to any page-level listener via a typed CustomEvent.
    window.dispatchEvent(new CustomEvent('sp:new_message', { detail: msg }));
  }, []);
  ...
  // dashboard. Child pages read livePositions / connected via useDashboardWs()
  // instead of calling useDashboardWebSocket() directly, which would open a
  // second connection on every navigation.
  return (
    <DashboardWsProvider onNewMessage={handleNewMessage}>
      <div className="flex h-screen overflow-hidden bg-slate-50">
        ...
        {/* Global sticky message notification popup — present on all dashboard pages */}
