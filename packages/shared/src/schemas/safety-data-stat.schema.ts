 ...
 * Credibility & Safety-Data Page (`docs/SafePassLanding/screens/05-credibility-page.md`).
 *
 * `source` and `asOfDate` are REQUIRED, not decorative. risk_log.md R-007
 ...
 * `VerificationStatusEnum` (see `incident.schema.ts`) — schema.md's
 * `SafetyDataStat.verificationTier` deliberately lists only four tiers, since
 * a rejected report is not something the site would ever publish as a stat.
 */
...
export const SafetyDataStatSchema = z.object({
  ...
  /** Date the figure was last confirmed accurate; rendered visibly so staleness is apparent (R-007). */
